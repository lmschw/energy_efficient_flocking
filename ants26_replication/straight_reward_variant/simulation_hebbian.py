...
def simulate_hebbian_episode(abcd_rules, seed=None, n_agents=None, wind_enabled=True,
    ...
    if record_trajectory or record_battery or record_wind_exposure:
        ...
        return dist_travelled, average_batt, collision_time, wall_collision_time, y_drift, telemetry
    return dist_travelled, average_batt, collision_time, wall_collision_time, y_drift
...
def stage_fitness(dist_travelled, average_batt, collision_time, y_drift, stage):
    ...
    identical failure mode) AND a straightness weight (config.HEBBIAN_STRAIGHTNESS_WEIGHT)
    replacing the original wall-collision penalty entirely -- see that constant's comment
    for the reasoning. Neither term is in the paper's literal Table 2 formula; both are
    deliberate, disclosed deviations.

    eff = HEBBIAN_EFF_DISTANCE_WEIGHT*dist - HEBBIAN_STRAIGHTNESS_WEIGHT*y_drift
          + avg_batt/battery_w - collision_time/collision_w
    """
    battery_w, collision_w, include_inter_robot = config.HEBBIAN_STAGE_FITNESS_WEIGHTS[stage]
    eff = (config.HEBBIAN_EFF_DISTANCE_WEIGHT * dist_travelled
           - config.HEBBIAN_STRAIGHTNESS_WEIGHT * y_drift)
    if battery_w is not None:
        eff += average_batt / battery_w
    if collision_w is not None and include_inter_robot:
        eff -= collision_time / collision_w
    return eff
