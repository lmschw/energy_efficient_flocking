"""Sweeps optimize_hebbian.py (this directory's walk_upwind variant -- see config.py)
across multiple swarm sizes and multiple seeds per size, to check whether/how results
generalize across agent counts rather than trusting a single (n_agents, seed)
combination.
...
../experiment/run_agent_count_sweep.py (same mechanism), with defaults switched to
this directory's own stage names.

Usage:
    python run_agent_count_sweep.py --wind-grid 50 --output-dir ../../hebbian_results_v2_upwind_sweep
...
def main():
    ...
    parser.add_argument("--stages", nargs="+", default=["walk_upwind", "save_battery_avoid_wall",
                                                          "save_battery_avoid_all"],
                         help="Passed through to optimize_hebbian.py --stages (default: this "
                              "variant's full 3-stage curriculum).")
    parser.add_argument("--output-dir", default="hebbian_results_v2_upwind_sweep",
