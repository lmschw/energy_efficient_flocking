...
import config
from battery_plot import plot_battery_levels
from hebbian_controller import unflatten_abcd
from simulation_hebbian import render_hebbian_episode_video
...
def _infer_wind_enabled(genome_path, history):
    """All three stages in this variant train with wind enabled (config.
    HEBBIAN_STAGE_WIND_ENABLED) -- unlike ../experiment/, where stage 1 trains wind-free.
    Prefers the history file's recorded 'stage'
...
if __name__ == "__main__":
    ...
    parser.add_argument("--seed", type=int, default=config.HEBBIAN_DEFAULT_SEED, help="Random seed for the run.")
    ...
    parser.add_argument("--wind", choices=["auto", "on", "off"], default="auto",
                         help="Whether wind is enabled during playback. 'auto' (default) infers this "
                              "from the genome filename's stage name (all three stages in this "
                              "variant -> on) -- override explicitly if you renamed the file, since "
                              "the wrong setting silently produces misleading playback rather than "
                              "an error.")
