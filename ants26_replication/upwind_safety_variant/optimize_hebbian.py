...
disabled) -- see config.py's module docstring for the full rationale. Everything else
below (CMA-ES settings, curriculum mechanism, stages 2/3) is unchanged from ../experiment/.
...
def train_one_seed(seed, output_dir, stages, popsize, maxiter, n_agents, n_repeats,
                    battery, wind_grid, no_battery_sensor, init_genome_path=None):
    ...
    init_genome_path: OPTIONAL, default None (unchanged default behavior). If given,
    the first stage in `stages` is seeded from this saved genome instead of a fresh
    random one -- e.g. to re-run just one stage with a different --popsize/--maxiter
    against an existing, unchanged earlier stage's result, without re-running that
    earlier stage or touching its output directory at all."""
...
if __name__ == "__main__":
    ...
    parser.add_argument("--init-genome", default=None, metavar="PATH",
                         help="Seed the FIRST stage in --stages from this existing genome .npy file "
                              "instead of a fresh random one. Default: not set, i.e. no change to the "
    ...
    if args.seeds is not None:
        ...
        train_one_seed(args.seed, args.output_dir, args.stages, args.popsize, args.maxiter,
                       ...
                       args.no_battery_sensor, init_genome_path=args.init_genome)
