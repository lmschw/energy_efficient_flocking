"""Sweeps optimize_hebbian.py (this directory's wall-sensor variant) across multiple
swarm sizes and multiple seeds per size, to check whether/how results generalize across
agent counts rather than trusting a single (n_agents, seed) combination.
...
optimize_hebbian.py's internals -- it only depends on its existing CLI. Kept in this
directory (not touching ../experiment/ or optimize_hebbian.py itself) so the sweep
mechanism can change without risk to the working code either variant depends on.

Usage:
    python run_agent_count_sweep.py --wind-grid 50 --output-dir ../../hebbian_results_v2_wallsensor_sweep
...
DEFAULT_N_AGENTS_LIST = [3, 5, 7, 9, 11, 13, 20]
# Same 10 canonical seeds optimize_hebbian.py's own --seeds (no explicit values) uses --
...
DEFAULT_SEEDS = [42, 123, 777, 2026, 888, 99, 412, 555, 1010, 8432]
...
def main():
    ...
    parser.add_argument("--seeds", type=int, nargs="+", default=DEFAULT_SEEDS,
                         help=f"Seeds to repeat per agent count (default: the 10 canonical "
                              f"seeds, {DEFAULT_SEEDS}).")
    parser.add_argument("--stages", nargs="+", default=["walk_left", "save_battery_avoid_all"],
                         help="Passed through to optimize_hebbian.py --stages (default: the "
                              "2-stage curriculum this variant was built to test).")
    parser.add_argument("--output-dir", default="hebbian_results_v2_wallsensor_sweep",
