...
# --- Neural controller architecture (Section 2.1, EXTENDED beyond the paper) ---
# +4 vs. the paper's original 10: this variant adds a wall-proximity input per quadrant
# (front/back/left/right), because the trained controller has no way to sense the
# simulation boundary at all otherwise -- get_sensor_data() only senses OTHER AGENTS,
# so wall-avoidance can only be learned indirectly through the fitness penalty. Real
# data showed this indirect learning working in the paper's original 3-stage curriculum
# (dedicated wall-only stage 2) but failing in a collapsed 2-stage variant (agents drove
# straight into the wall and got stuck there for most of the episode, unaffected by more
# CMA-ES generations or population size -- a genuine exploration/credit-assignment
# plateau, not an optimization-budget problem). This is a disclosed, deliberate
# deviation from the paper's stated 10-input architecture -- document it as such in any
# writeup. Also motivated by hardware deployment: unlike neighbor-sensing (which needs
# OptiTrack -- see hardware_deployment/'s sensing caveat), wall proximity in a bounded
# arena is within real Thymio IR range, so this input could plausibly be read from the
# real onboard sensor instead of a mocap substitute.
HEBBIAN_N_INPUTS = 14             # 4 quadrants x (distance, bearing) neighbor-sensing
                                   # + 4 quadrants x distance-only wall-sensing
                                   # + battery + compass heading
