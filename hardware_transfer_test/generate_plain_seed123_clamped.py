"""Generates hardware_transfer_test/upwind_2stage_plain_seed123_clamped/n<N>/{video.mp4,
battery_plot.png,metrics.json} for the clamped retrain of plain_seed123 (5/30 both-beat-
baseline at n=20 -- the weakest clamped survivor on that axis, but plain_seed123 is also the
only one of the 4 winning genomes that doesn't chronically hug/stick to a wall -- see
project_upwind_safety_variant.md's 2026-09-16 updates). Evaluated WITH the safety clamp
active (HEBBIAN_SAFETY_CLAMP_ENABLED=True, MIN_DIST_INFLATION=1.3, RESOLVE_COLLISIONS=True
strength=0.5 max_iter=2), matching how it was both trained and 30-seed-evaluated.

Usage: python generate_plain_seed123_clamped.py
"""
import json
import os
import sys
...
LABEL = "upwind_2stage_plain_seed123_clamped"
