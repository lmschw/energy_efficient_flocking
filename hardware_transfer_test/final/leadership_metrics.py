"""Leading/following turn-taking metrics from the collective-motion literature, applied to the
FINAL chosen genome from the "beat the LJ baseline" + safety-clamp investigation --
plain_seed123 (unclamped) and plain_seed123_clamped (retrained from scratch with the hard
safety clamp active) -- against the LJ baseline, across n_agents in {2,3,4,5,7,10,20}. A
fork of leadership_metrics.py (which covers the original pre_clamp_best/safety_clamp_best/
lj_baseline three-condition comparison) kept deliberately separate/isolated per the user's
request, rather than folding this new genome into that earlier pipeline.
...
saved genomes/rules instead using each condition's own known-correct config overrides (see
_CONDITION_OVERRIDES below) and wind grid (50x50, matching this recipe's training-time
--wind-grid 50).
...
N_AGENTS_SWEEP = (2, 3, 4, 5, 7, 10, 20)
SEED = config.HEBBIAN_DEFAULT_SEED  # 42
WIND_GRID = 50  # matches this recipe's training-time wind grid (--wind-grid 50)
OUTPUT_DIR = os.path.join(SCRIPT_DIR, "leadership_analysis")

# plain_seed123: the 2-stage-upwind winning genome chosen to carry forward (see
# project_upwind_safety_variant.md, 2026-09-16 final decision) -- both its unclamped
# (best raw performance) and clamped-from-scratch-retrained (hardware-safe) forms, compared
# side by side against the LJ baseline. Kept in its own module/output dir (isolated from
# leadership_metrics.py's original pre_clamp_best/safety_clamp_best/lj_baseline comparison)
# per the user's request to isolate this genome's analysis rather than fold it into the
# earlier three-condition pipeline.
_CONDITION_OVERRIDES = {
    "plain_seed123": dict(safety_clamp=False, min_dist_inflation=1.0, resolve_collisions=False),
    "plain_seed123_clamped": dict(safety_clamp=True, min_dist_inflation=1.3, resolve_collisions=True,
...
CONDITION_LABELS = {
    "plain_seed123": "plain_seed123 (unclamped)",
...
BATTERY_SCALE = {  # metrics.json's "batt_pct" = average_batt / this * 100 -- see module docstring
    ...
    "plain_seed123": config.HEBBIAN_MAX_BATTERY,   # 100.0
    "plain_seed123_clamped": config.HEBBIAN_MAX_BATTERY,
}
