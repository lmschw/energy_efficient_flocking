...
Efficient Flocking in Self-Organized Robot Swarms") for the FINAL chosen genome
(plain_seed123, unclamped and clamped) vs. the LJ baseline -- a fork of paper_figures.py
(which covers the original pre_clamp_best/safety_clamp_best/lj_baseline comparison), using
leadership_metrics_plain_seed123.py's condition set instead.
...
def fig5a_distance_vs_battery(n_agents, seeds, out_dir):
    ...
    _savefig(fig, out_dir, f"paper_fig5a_distance_vs_battery_n{n_agents}_plain_seed123.png")
...
def fig6_trajectory_comparison(agent_counts, seed, out_dir):
    ...
    fig.suptitle(f"Fig. 6 analog -- trajectories at {agent_counts[0]} vs. {agent_counts[1]} agents "
                 ...
                 fontweight="bold")
    _savefig(fig, out_dir, "paper_fig6_trajectory_comparison_plain_seed123.png", rect=[0, 0, 1, 0.95])
