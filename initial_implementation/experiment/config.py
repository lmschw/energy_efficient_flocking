"""Central configuration for the original LJ-model swarm simulation and CMA-ES
optimization scripts (the project this repo's ants26_replication/ later grew out of).
...
This file used to also hold the Hebbian ABCD replication's constants; they've moved to
../../ants26_replication/experiment/config.py as an independent copy (some physics
constants are duplicated between the two files by necessity -- see that file's
docstring for why). This file has no dependency on that project.
"""
...
# from the paper's literal baseline (epsilon=1, no U term). Used by
# ants26_replication/experiment/analyze_hebbian_results.py (via its own lj_baseline.py
# copy, not this file).
...
def genome_to_rules(genome):
    ...
    if len(genome) != len(GAIN_NAMES):
        raise ValueError(
            ...
            f"{len(GAIN_NAMES)} LJ gains ({GAIN_NAMES}). A length of 880 means this is a "
            f"Hebbian ABCD genome (from ants26_replication/experiment/optimize_hebbian.py) "
            f"-- those don't work with visualize_best.py/genome_to_rules at all; use "
            f"ants26_replication/experiment/analyze_hebbian_results.py or "
            f"simulate_hebbian_episode() instead.")
